...
create table Battle (id bigint generated by default as identity, endTime timestamp, startTime timestamp, winnerId bigint not null, FIRST_TRAINER_ID bigint not null, SECOND_TRAINER_ID bigint not null, primary key (id))
...
create table RECORD_BATTLES (RECORD_ID bigint not null, BATTLE_ID bigint not null, primary key (RECORD_ID, BATTLE_ID))
create table Record (id bigint generated by default as identity, badgesWon integer not null, battlesLost integer not null, battlesWon integer not null, competitionWins integer not null, primary key (id))
create table TRAINER_CONTACTS (TRAINER_ID bigint not null, OTHER_ID bigint not null, primary key (TRAINER_ID, OTHER_ID))
create table Trainer (id bigint generated by default as identity, dateOfBirth date, firstName varchar(255), gender varchar(255), lastName varchar(255), record_id bigint not null, primary key (id))
create table User (_id bigint not null, _firstname varchar(255), _lastname varchar(255), _username varchar(255), primary key (_id))
alter table Trainer add constraint UK_td0nuspjhvreqjt22l08skepn  unique (record_id)
...
alter table RECORD_BATTLES add constraint FK_3tmi6lpb35l5gy69l006tvf9h foreign key (BATTLE_ID) references Battle
alter table RECORD_BATTLES add constraint FK_gllu6fmgakt7ware3orbylgkx foreign key (RECORD_ID) references Record
...
alter table Trainer add constraint FK_td0nuspjhvreqjt22l08skepn foreign key (record_id) references Record
